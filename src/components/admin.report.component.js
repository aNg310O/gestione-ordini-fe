...
const AdminReportTable = () => {
  ...
  const getData = async () => {
    var totalone = 0;
    try {
      const res = await API.get(`/gestione-ordine/todayOrder`, {
        headers: authHeader(),
      });
      if (res.status === 200) {
        if (seller.roles[0] === "ROLE_ADMIN") {
          ...
          setLoading(false);
          console.log(
            `INFO, ${seller.username}, admin.report.component, getData todayOrder`
          );
          Logging.log(
            "INFO",
            seller.username,
            "admin.report.component",
            `WEB - get today order report`
          );
        }
      }
    } catch (e) {
      if (e.message === "Network Error") {
        ...
        setOpen(true);
        console.log(
          `ERROR, ${seller.username}, admin.report.component, getData error ${e.message}`
        );
        Logging.log(
          "ERROR",
          seller.username,
  ...
  const handleReportClick = () => {
    ...
    if (
      ...
      console.log(
        `INFO, ${seller.username}, admin.report.component, handleReportClick download from browser`
      );
      Logging.log(
        "INFO",
        seller.username,
      ...
      console.log(
        `INFO, ${seller.username}, admin.report.component, handleReportClick download from mobile`
      );
      Logging.log(
        "INFO",
        seller.username,
