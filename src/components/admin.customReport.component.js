...
const AdminReportDay = () => {
  ...
  const getData = async (str) => {
    try {
      var totalone = 0;
      const res = await API.get(`/gestione-ordine/dateOrder/${str}`, {
        headers: authHeader(),
      });
      if (res.data.length !== 0) {
        ...
        setEmpty(false);
        console.log(
          `INFO, ${seller.username}, admin.customReport.component, getData dateOrder`
        );
        Logging.log(
          "INFO",
          seller.username,
          "admin.customReport.component",
          `WEB - Eseguito report per la data ${str}`
        );
      } else {
        setEmpty(true);
        setMsg(
        ...
        console.log(
          `INFO, ${seller.username}, admin.customReport.component, getData not today order yet`
        );
        Logging.log(
          "INFO",
          seller.username,
          "admin.customReport.component",
          `WEB - Eseguito report per la data ${str}, nessun risultato trovato`
        );
      }
    } catch (e) {
      if (e.message === "Network Error") {
        ...
        setOpen(true);
        console.log(
          `ERROR, ${seller.username}, admin.customReport.component, getData error ${e.message}`
        );
        Logging.log(
          "ERROR",
          seller.username,
  ...
  const handleReportClick = () => {
    ...
    var test = new jsPDF();
    test.text(
      `Report ordini per il giorno ${fileName.substring(7, 16)}`,
      10,
      15
    );
    ...
    if (
      ...
      console.log(
        `INFO, ${seller.username}, admin.customReport.component, handleReportClick download from browser`
      );
      Logging.log(
        "INFO",
        seller.username,
      ...
      console.log(
        `INFO, ${seller.username}, admin.customReport.component, handleReportClick download from mobile`
      );
      Logging.log(
        "INFO",
        seller.username,
  ...
  const renderBody = () => {
    return (
      ordine &&
      ordine.map(({ _id, qty, totale, dataInserimento }) => {
        return (
          <tr key={_id.desc}>
            <td>{_id.desc}</td>
  ...
  if (empty) {
    return (
      <div id="root-content">
        <TextField
          style={{ backgroundColor: "#6d6c6c" }}
          InputLabelProps={{ shrink: true }}
          InputProps={{ readOnly: true }}
          variant="outlined"
          value="Scegli la data"
        />
