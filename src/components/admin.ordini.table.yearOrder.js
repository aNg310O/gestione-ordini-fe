...
const AdminYearOrder = () => {
  ...
  const retrieveMonth = async () => {
    try {
      const response = await API.get("gestione-ordine/availableYear", {
        headers: authHeader(),
      });
      if (response.status === 200) {
        ...
        setLoading(false);
        console.log(
          `INFO, ${seller.username}, ordini.table.yearOrder.component, retrieveYear() Call OK`
        );
        Logging.log(
          "INFO",
          seller.username,
  ...
  const getData = async (str) => {
    try {
      const res = await API.get(`/gestione-ordine/monthyearReport/${str}`, {
        headers: authHeader(),
      });
      if (res.data.length !== 0) {
        ...
        //setEmpty(false);
        console.log(
          `INFO, ${seller.username}, ordini.table.yearOrder.component, getData monthyearReport`
        );
        Logging.log(
          "INFO",
          seller.username,
        ...
        console.log(
          `INFO, ${seller.username}, ordini.table.yearOrder.component, getData month order empty`
        );
        Logging.log(
          "INFO",
          seller.username,
  ...
  const handleReportClick = () => {
    ...
    if (
      ...
      console.log(
        `INFO, ${seller.username}, ordini.table.yearOrder.component, handleReportClick download from browser`
      );
      Logging.log(
        "INFO",
        seller.username,
      ...
      console.log(
        `INFO, ${seller.username}, ordini.table.yearOrder.component, handleReportClick download from mobile`
      );
      Logging.log(
        "INFO",
        seller.username,
  ...
  const renderBody = () => {
    return (
      ordine &&
      ordine.map(({ _id, desc, pesoProdotto, pezzatura, totale, qty }) => {
        return (
          <tr key={_id}>
            <td>{_id.desc}</td>
        ...
        <FormControl className={classes.formControl}>
          ...
          >
            {tests &&
              tests.map((myData) => (
                <MenuItem
                  value={myData.dataInserimento}
                  key={myData.dataInserimento}
                >
                  {myData.dataInserimento}
                </MenuItem>
              ))}
          </Select>
        </FormControl>
